...
static uintptr_t _get_pc_for_thread(thread_t thread) //获得目标线程的寄存器状态
#if defined(__i386__)
{
...
#elif defined(__x86_64__)
{
    ...
    kern_return_t okay = thread_get_state (thread, x86_THREAD_STATE64, (thread_state_t)&state, &count); //获取线程的寄存器状态
    return (okay == KERN_SUCCESS) ? state.__rip : PC_SENTINEL; //rip是PC在x86-64架构下的名字，其中R代表"register",IP代表"instruction pointer";
}
...
static int _collecting_in_critical(void) //它在释放残留的内存垃圾之前调用.runtime实际上有两种独立的模式：一种是留下垃圾知道下次再有其他线程进入临界函数。另一个是不断的循环直到清除干净，而且通常会同时释放这些垃圾内存。
{
#if TARGET_OS_WIN32
    return TRUE;
#else
